...
def execute(filters=None):
	...
	process_list_for_item = get_process_list_for_item(filters.get("item_filter"))
	...
	#print "process_column_bind_list", process_column_bind_list

	#items
	item_list =[filters.get("item_filter") ] #time being goes with only one item group
	non_process_colum_name_list = ["item_code", "manufacturing_method"]
...
def get_columns(filters):
	columns = []
	process_list = get_process_list_for_item(filters.get("item_filter"))  #for single item
...
#for single item
def get_process_list_for_item( item_code ):
	method_name = frappe.db.get_value("Pch Manufacturing Method Child", {"item_made":item_code},"parent")
...
def get_global_process_order_list():
	process_dic = frappe.db.sql("""
		SELECT 
	    pch_process,pch_process_order
		FROM
	    `tabPch Global Process Child`
		order by 
		pch_process_order; """, as_dict=1)
